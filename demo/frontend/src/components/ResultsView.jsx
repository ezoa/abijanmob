...
export default function ResultsView({ plan, selected, onSelect, onPay, onBack, stopMap, network, vehicles }) {
  const [open, setOpen] = useState(selected?.id || null)
  const transit = (it) => it.legs.some((l) => l.type === 'ride' && l.mode !== 'taxi')
  const liveLeg = (it) =>
    it.legs.find(
      (l) => l.type === 'ride' && l.mode !== 'taxi' && (vehicles || []).some((v) => v.line_id === l.line_id)
    )
      ...
      <div className="res-head">
        ...
        <div>
          ...
          <div className="res-sub">
            {plan.itineraries.length} options · formel & informel comparés · véhicules en direct
          </div>
        </div>
      </div>

      <div className="res-map">
        <MapView network={network} itinerary={selected} plan={plan} vehicles={vehicles} dim />
      </div>

      <div className="res-list">
        {plan.itineraries.map((it) => {
          const live = liveLeg(it)
          return (
            <div
              key={it.id}
            ...
            >
              ...
              {live && (
                <LiveChip
                  lineId={live.line_id}
                  stopId={live.from}
                  stopName={stopMap[live.from]?.name}
                  who={live.line_name}
                />
              )}
        ...
        <div className="disclaimer-mini">
          Itinéraires indicatifs (prototype) · tarifs et fréquences à valider terrain · suivi
          véhicules : simulation accélérée ×4
        </div>
      </div>
    </div>
  )
